...
//approve_paid_contracts($_GET['id']);
function approve_paid_contracts($subcontractors_invoice_setup_id){
	...
	foreach($subcontractors_invoice_setup_details as $detail ){
		...
		if($subcontractors_temp['id'] != ''){
			...
			$sql = $db->select()
				 ...
				 ->join(array('l' => 'leads'), 'l.id = s.leads_id', Array('client_fname' => 'fname', 'client_lname' => 'lname', 'client_email' => 'email', 'company_name', 'registered_domain'))
			...
			if(! TEST){
				//admin email
				$to = 'ricag@remotestaff.com.au'; // replace rhiza@remotestaff.com.au
				$fullname = "Rica J.";
