...
def expire_mass_responder_code(userid, mass_responder_code):
    logging.info('Executing schedule.expire_mass_responder_code (%s , %s) ' % (userid, mass_responder_code) )
    conn = engine.connect()

    #get existing mass_responder_code
    sql = text("""SELECT mass_responder_code FROM personal WHERE userid=:userid""")
    r = conn.execute(sql, userid=userid).fetchone()
    if r[0] == mass_responder_code:
        sql = text("""UPDATE personal SET mass_responder_code=NULL WHERE userid=:userid""")
...
def run_tests():
    ...
    >>> expire_mass_responder_code(69, 'FAIL')
    >>> expire_mass_responder_code(28811, 'fffa69ff84bbb0ce4674cac984293bb339153f17')
