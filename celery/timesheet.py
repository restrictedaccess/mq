...
def get_invoice_items(client_id, month_year_str):
    ...
    for month_year in month_years:
        timesheets = conn.execute(sql, client_id=client_id, month_year=month_year['date_str']).fetchall()
        ...
        for ts in timesheets:
            ...
            subcon_id = ts.subcontractors_id
            staff_name= '%s %s' % (ts.fname, ts.lname)
            job_designation = '%s' % ts.job_designation
...
def get_time_records_between(conn, timerecords, start_date_time, end_date_time, subcontractors_id, timezone_ref, userid, leads_id, month_year, day, timesheet_details_id):
    ...
    sql = text("""
        ...
        AND l.userid = :userid
        AND l.leads_id = :leads_id
        AND date_of_leave BETWEEN :leave_request_start_date AND :leave_request_end_date
    """)
    ...
    sql = text("""
        SELECT t.note, t.timestamp,t.working_hrs,t.notes_category,t.file_name,p.fname, p.lname, t.id
        FROM timesheet_notes_subcon AS t
        JOIN personal AS p
        ON t.userid = p.userid
        WHERE timesheet_details_id = :timesheet_details_id
    """)
    data = conn.execute(sql, timesheet_details_id=timesheet_details_id)
    for d in data:
        x = d.timestamp
        y = datetime(x.year, x.month, x.day, x.hour, x.minute, x.second, tzinfo=phtz)
        timestamp = y.astimezone(timezone_ref)
    ...
    sql = text("""
        SELECT t.note, t.timestamp, a.admin_fname AS fname, a.admin_lname AS lname, t.id
...
def get_timesheet_details(timesheet_id, timezone_ref=None):
    ...
    conn = engine.connect()
    sql = text("""
        SELECT t.month_year, t.userid, t.leads_id, t.status AS timesheet_status, tz.timezone, t.subcontractors_id
        FROM timesheet AS t
        LEFT JOIN timezone_lookup AS tz
        ON t.timezone_id = tz.id
        WHERE t.id = :timesheet_id
    """)
    timesheet = conn.execute(sql, timesheet_id=timesheet_id).fetchone()
    ...
    """)
    prepaid = conn.execute(sql, subcontractors_id = subcontractors_id).fetchone()[0]
    ...
    sql = text("""
        ...
        WHERE timesheet_id = :timesheet_id
    """)
    timesheet_details = conn.execute(sql, timesheet_id=timesheet_id).fetchall()
...
def get_timesheet_notes(timesheet_details_id, timezone_ref):
    ...
    data = conn.execute(sql, timesheet_details_id=timesheet_details_id).fetchone()
    ...
    sql = text("""
        ...
        WHERE id = :ts_id
    """)
    data = conn.execute(sql, ts_id=ts_id).fetchone()
    ...
    #notes from timesheet_notes_subcon
    sql = text("""
        SELECT t.note, t.timestamp, p.fname, p.lname
        FROM timesheet_notes_subcon AS t
        JOIN personal AS p
        ON t.userid = p.userid
        WHERE timesheet_details_id = :timesheet_details_id
